...
| File | Role |
|---|---|
| `app.py` | Gradio entry point. Builds the 5-tab interface. |
| `gradio_app/schema.py` | Single source of truth for the ~50 hyperparameters. |
| `gradio_app/widgets.py` | Auto-generates widgets from the schema; wires dependency enable/disable. |
| `gradio_app/runners.py` | Thin programmatic wrappers around `train`/`test`/`NeuroSimFitter`/`NeuroSimOptimizer`. |
| `gradio_app/config_io.py` | JSON config export/import. |
| `config.py` | Original default values. |
| `train_eval.py`, `neurosim_utils.py`, `data_loader.py`, `models.py` | Core compute modules — unchanged by the UI layer. |
